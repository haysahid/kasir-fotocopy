...
import DeleteEmployeeConfirmation from "./DeleteEmployeeConfirmation.vue";
...
function onItemFormDialogClosed(value) {
    ...
    if (value) {
        employeeStore.query.page = employeeStore.data.current_page;
        getData();
    }
...
async function getData(params) {
    await employeeStore.getAllItems(params);
}

async function disableOrEnableItem(item) {
    await employeeStore.disableOrEnableItem(item);
}

const changePage = (page) => {
    employeeStore.query.page = page;
...
watch(
    ...
    (newValue, oldValue) => {
        console.log("current page: ", employeeStore.data.current_page);
    }
);
    ...
    (newValue, oldValue) => {
        if (!oldValue && newValue) {
            selectedItems.value = JSON.parse(
                JSON.stringify(
                    employeeStore.items.filter((i) => i.role_id >= 3)
                )
            );
        }
        ...
        if (
            oldValue &&
            !newValue &&
            selectedItems.value.length === employeeStore.items?.length
        ) {
            selectedItems.value = [];
        }
    }
    ...
    >
        ...
        <div class="flex flex-col overflow-x-auto">
            <table class="table-fixed">
                ...
                <tbody>
                    <tr
                        v-for="(item, key) in employeeStore.items"
                        :key="key"
                        ...
                        :class="{
                            'border-b border-stroke dark:border-strokedark':
                                key !== employeeStore.items.length - 1,
        ...
        >
            <vue-awesome-paginate
                v-if="employeeStore?.data?.current_page"
                :total-items="employeeStore.data.total"
                :items-per-page="employeeStore.data.per_page"
                :max-pages-shown="5"
                v-model="employeeStore.data.current_page"
                @click="changePage"
            />
            <p class="text-xs font-light text-gray-400">
                Showing {{ employeeStore.data?.from }} to
                {{ employeeStore.data?.to }} of
                {{ employeeStore.data?.total }} entries
            </p>
        </div>
