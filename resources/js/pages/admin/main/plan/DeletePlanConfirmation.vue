...
async function deleteItems() {
    const result = await adminUserStore.deleteItems(props.items);
    ...
    >
        <div class="px-6.5 py-8 flex flex-col items-center">
            ...
            >
                <CustomButton
                    @click="close(false)"
                    :enable="adminUserStore.deleteStatus !== 'loading'"
                    color="bg-slate-400"
                    :is-full="true"
                    padding="py-2.5 px-6"
                >
                ...
                <CustomButton
                    @click="deleteItems"
                    :loading="adminUserStore.deleteStatus === 'loading'"
                    :loading-text="`${adminUserStore.deleteProgress}/${props.items.length}`"
                    color="bg-danger"
                    :is-full="true"
                    padding="py-2.5 px-6"
                >
