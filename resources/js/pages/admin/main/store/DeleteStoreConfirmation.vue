...
async function deleteItems() {
    const result = await storeStore.deleteItems(props.items);
    ...
    >
        <div class="px-6.5 py-8 flex flex-col items-center">
            ...
            >
                <CustomButton
                    @click="close(false)"
                    :enable="storeStore.deleteStatus !== 'loading'"
                    color="bg-slate-400"
                    :is-full="true"
                    padding="py-2.5 px-6"
                >
                ...
                <CustomButton
                    @click="deleteItems"
                    :loading="storeStore.deleteStatus === 'loading'"
                    :loading-text="`${storeStore.deleteProgress}/${props.items.length}`"
                    color="bg-danger"
                    :is-full="true"
                    padding="py-2.5 px-6"
                >
