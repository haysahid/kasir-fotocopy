...
async function deleteItems() {
    const result = await subscriptionStore.deleteItems(props.items);
    ...
    >
        <div class="px-6.5 py-8 flex flex-col items-center">
            ...
            >
                <CustomButton
                    @click="close(false)"
                    :enable="subscriptionStore.deleteStatus !== 'loading'"
                    color="bg-slate-400"
                    :is-full="true"
                    padding="py-2.5 px-6"
                >
                ...
                <CustomButton
                    @click="deleteItems"
                    :loading="subscriptionStore.deleteStatus === 'loading'"
                    :loading-text="`${subscriptionStore.deleteProgress}/${props.items.length}`"
                    color="bg-danger"
                    :is-full="true"
                    padding="py-2.5 px-6"
                >
