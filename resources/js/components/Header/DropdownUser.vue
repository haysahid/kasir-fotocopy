...
<template>
  <div class="relative" ref="target">
    <router-link
      ...
      class="flex items-center gap-4 max-md:ml-2 max-lg:ml-5 max-lg:mt-2"
      to="#"
      @click.prevent="dropdownOpen = !dropdownOpen"
    >
      <div class="flex items-center gap-4 max-lg:flex-row-reverse">
        <span class="text-right max-lg:text-left">
          <span class="block text-sm font-medium text-black dark:text-white">{{
            props.user.realname
          }}</span>
          <span class="block text-xs font-medium"
            >{{ props.user.username }} -
            {{ props.user.role.nama.toLowerCase().replace("role_", "") }}</span
          >
        </span>
