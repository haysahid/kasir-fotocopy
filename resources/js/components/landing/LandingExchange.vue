<template>
  <div class="flex items-center space-x-4">
    ...
    <div class="relative w-full max-w-[106px] sm:max-w-[159px]">
      ...
      >
        <img
          :src="`./src/assets/img/${exchangeSelected.img}`"
          alt=""
          class="flex-shrink-0 w-6 h-6 rounded-full"
        />
        <span class="block ml-3 truncate">{{ exchangeSelected.name }}</span>
      ...
      <transition name="transform-fade-down">
        ...
        >
          <li
            ...
            id="listbox-option-0"
            :key="exchange.name"
            class="relative px-3 py-2 text-gray-900 cursor-default select-none sm:px-5"
            role="option"
          >
