...
export const usePaymentMethodStore = defineStore('payment-method', () => {
    ...
    async function getItem(id) {
        ...
        try {
            ...
            const response = await axios.get(`/api/payment-method/${id}`, {
                headers: { Authorization: token }
            });
    ...
    async function addItem(item) {
        ...
        try {
            const response = await axios.post("/api/payment-method", item, {
                headers: { Authorization: token },
            });
    ...
    async function updateItem(id, item) {
        ...
        try {
            const response = await axios.put(`/api/payment-method/${id}`, item, {
                headers: { Authorization: token },
            });
    ...
    async function disableOrEnableItem(item) {
        try {
            ...
            const response = await axios.put(
                `/api/payment-method/${item.id}/${action}`,
                {},
                {
                    headers: {
                        Accept: "application/json",
    ...
    async function getDropdown(search) {
        try {
            const response = await axios.get("/api/payment-method-dropdown", {
                headers: { Authorization: token },
                params: { search },
            });
    ...
    function clearPaymentMethodStore() {
        data.value = {}
        query.value = {
            limit: 10,
            page: 1,
