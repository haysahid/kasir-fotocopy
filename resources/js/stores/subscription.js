...
export const useSubscriptionStore = defineStore('subscription', () => {
    ...
    async function addItem(item) {
        ...
        try {
            const response = await axios.post("/api/subscription", item, {
                headers: { Authorization: token },
            });
    ...
    async function updateItem(id, item) {
        ...
        try {
            const response = await axios.put(`/api/subscription/${id}`, item, {
                headers: { Authorization: token },
            });
    ...
    async function disableOrEnableItem(item) {
        try {
            ...
            const response = await axios.put(
                `/api/subscription/${item.id}/${action}`,
                {},
                {
                    headers: {
                        Accept: "application/json",
    ...
    async function setPriority(item) {
        try {
            ...
            const response = await axios.put(
                `/api/subscription/${item.id}/priority/${value}`,
                {},
                {
                    headers: {
                        Accept: "application/json",
    ...
    async function getDropdown(search) {
        try {
            ...
            const response = await axios.get("/api/subscription-dropdown", {
                headers: { Authorization: token },
                params: { search },
            });
    ...
    function clearSubscriptionStore() {
        data.value = {}
        query.value = {
            limit: 10,
            page: 1,
